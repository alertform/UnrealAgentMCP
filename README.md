...
- **BehaviorTree authoring** (1.3-dev): `read_bt` reads the full runtime node tree with decorators/services/key props; `add_bt_node` creates Composite or Task nodes at any index-path depth with property import and FBlackboardKeySelector resolution; `add_bt_decorator` attaches decorators to child slots with observer_aborts (EBTFlowAbortMode) and BlackboardKey resolution. BTGraph is nulled after writes so the editor rebuilds from the runtime tree on next open (confirmed via FBehaviorTreeEditor::RestoreBehaviorTree). New module dependency: AIModule.
- **AnimGraph authoring** (1.3-dev): `add_anim_graph_node` inserts any UAnimGraphNode_Base subclass (e.g. AnimGraphNode_Slot, AnimGraphNode_LayeredBoneBlend) into an Animation Blueprint's AnimGraph; supports `Node.PropertyName` dot-path for runtime struct members (e.g. `{"Node.SlotName":"UpperBody"}`); returns node GUID + pins list for immediate `connect_pins` chaining. `register_skeleton_slot` registers a slot name on a USkeleton (idempotent); required before placing non-default Slot nodes. Primary use-case: upper-body layered-blend setup (attack montage on upper body, locomotion on full body). New module dependencies: AnimGraph, AnimGraphRuntime.
- **Material authoring** (1.3-dev): `create_material` (UMaterialFactoryNew under /Game) + `add_material_expression` (any UMaterialExpression subclass — short name resolved via the `/Script/Engine` path, no TryFindType short-name warning) + `set_material_expression_property` (ImportText: Constant colors, ScalarParameter name/default, TextureSample texture by path) + `connect_material_expression` (node→node) + `connect_material_property` (node→BaseColor/Metallic/Roughness/Normal/EmissiveColor/…) + `recompile_material` + `describe_material` (reads the graph straight from the asset via `GetExpressionInputForProperty`/`GetInput` — works headless with no open material editor). Wraps `UMaterialEditingLibrary`; the save→reload round-trip is locked with the LOAD_ForDiff bytes-reload technique (`UnrealAgentMCP.MaterialTools.*`). New module dependency: MaterialEditor.
- **Material instances + assignment** (1.3-dev): `create_material_instance` (a `UMaterialInstanceConstant` parented to any Material/MIC via `UMaterialInstanceConstantFactoryNew`) + `set_material_instance_parameter` (scalar / vector `(R=..,G=..)` / texture-by-path / static-switch, then `UpdateMaterialInstance`) + `describe_material_instance` (parent + all overridable parameter names/values) + `assign_material` (puts a Material or MIC on a placed editor-world actor's mesh-component slot — retires the Python `set_material` escape hatch). Instance override round-trip locked via LOAD_ForDiff.
- **Niagara N1 — systems, user parameters, placement** (1.3-dev): `create_niagara_system` (empty system via `UNiagaraSystemFactoryNew`, constructed through the reflection registry — the factory class exports no API macro; the tool **blocks on `WaitForCompilationComplete`** before returning so no async compile task outlives the call — a leaked background compile crashed a headless run when a later `delete_asset` freed the system under it) + `set_niagara_user_parameter` (float/int/bool/linearcolor/vec3/vec2 into the exposed `FNiagaraUserRedirectionParameterStore`, `User.` prefix auto-normalized, add-if-missing) + `describe_niagara_system` (user params with live values + emitter handle names) + `place_niagara_component` (registered `UNiagaraComponent` on an editor-world actor). Emitter/module **graph** authoring is deliberately out of scope — no stable public API (editor ViewModel internals only); the M4 `add_niagara_emitter` spike gates on its own persistence test. New module dependencies: Niagara, NiagaraEditor; plugin dependency: Niagara.
